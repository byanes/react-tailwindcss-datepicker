...
const Calendar = ({ date, onClickPrevious, onClickNext, changeMonth, changeYear }) => {
    ...
    const clickDay = useCallback((day, month = date.month() + 1, year = date.year()) => {
        ...
        function chosePeriod(start, end) {
            ...
            changeDatepickerValue({
                startDate: require$$0(start).format("YYYY-MM-DD"),
                endDate: require$$0(end).format("YYYY-MM-DD")
            }, ipt);
            hideDatepicker();
        }
        if (period.start && period.end) {
...
const Input = (e) => {
    ...
    const handleInputChange = useCallback((e) => {
        ...
        if (start.length === 10 &&
            ...
            changeDayHover(require$$0(end).add(-1, "day").format(DATE_FORMAT));
            hideDatepicker();
            if (input) {
                input.blur();
            }
            ...
            const formatConfig = Object.keys(configs.shortcuts).map(item => {
                if (Object.keys(DEFAULT_SHORTCUTS).includes(item)) {
                    ...
                    if (configs.shortcuts && configs?.shortcuts[item]) {
                        ...
                        const start = require$$0(customConfig?.period?.start);
                        const end = require$$0(customConfig?.period?.end);
                        if (text &&
                            start.isValid() &&
                            end.isValid() &&
